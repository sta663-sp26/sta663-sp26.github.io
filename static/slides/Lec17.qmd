...
::: {.aside}
Either `run` or `dev` commands can be used for production vs development mode. The primary difference is the latter has auto-reload enabled which restarts the server when the underlying code changes. `dev` also binds `127.0.0.1` by default while `run` binds `0.0.0.0`.
:::
...
Query and path parameters are most natural with `GET` and `DELETE`. `POST`, `PUT`, and `PATCH` typically receive data via a request body.
...
#* @get /add
function(x, y = 0) {
  list(result = as.integer(x) + as.integer(y))
}
...
::: {.aside}
Key differences: FastAPI uses native Python decorators (vs comment-based); type hints drive validation automatically; async support; built-in interactive docs.
:::
...
::: {.xsmall}
```python
buf = io.BytesIO()        # in-memory binary buffer — no disk write needed
...
::: {.aside}
`matplotlib.use("Agg")` must be set before importing `pyplot` on the server to select a non-interactive backend (no display required).
:::

...
* `pystan` & `RStan` - native language interface to the underlying Stan C++ libraries
  ...
  * The former does not play nicely with Jupyter (or quarto or positron) - see [here](https://pystan.readthedocs.io/en/latest/faq.html#how-can-i-use-pystan-with-jupyter-notebook-or-jupyterlab) for a fix
...
::: {.aside}
Model and data from Martin Ingram's ["MCMC for big datasets -- faster sampling with JAX and the GPU"](https://martiningram.github.io/mcmc-comparison/); results based on [martiningram/mcmc_runtime_comparison](https://github.com/martiningram/mcmc_runtime_comparison) and are available in [rundel/mcmc_runtime_comparison](https://github.com/rundel/mcmc_runtime_comparison).
:::

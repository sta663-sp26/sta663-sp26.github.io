...
These are both variants of Newton's method but do not require the Hessian (optionally can be used by Newton-CG).
...
An alternative approach is to transform the parameter so it is unconstrained - here we optimize over `log(σ)` instead of `σ`, using `exp()` to map back to the positive reals.
...
res = optimize.minimize(
  mle_norm3, x0=[0, np.log(1)], method="bfgs"
)
res
```
:::

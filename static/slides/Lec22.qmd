...
These are both variants of Newton's method but do not require the Hessian (optionally can be used by the former).
...
* Rather than storing and updating a full $n \times n$ approximation to the inverse Hessian, L-BFGS stores only the last $m$ updates (typically $m \approx 5$--$20$) and uses these to represent the inverse Hessian
...
::: {.medium}
Trust-region methods take a fundamentally different approach from line search methods — instead of choosing a direction and then a step size, they define a region around the current point where the quadratic model of the objective function is trusted to be accurate.

* At each iteration, the method minimizes the quadratic model $f(x_k) + \nabla f(x_k)^T p + \frac{1}{2} p^T \nabla^2 f(x_k) \, p$ subject to $\|p\| \leq \Delta_k$ (the trust-region radius)

* The trust-region radius $\Delta_k$ is adapted based on how well the quadratic model predicted the actual function change

* The inner minimization is solved approximately using the conjugate gradient method (hence "NCG")

* Requires the gradient, and benefits significantly from the Hessian (approximated if not provided)

* Generally more robust than line search Newton methods, particularly near saddle points
:::

::: {.aside}
SciPy also provides `"trust-exact"` (exact trust-region solution, requires Hessian), `"trust-krylov"` (uses Krylov subspace methods for large-scale problems), and `"trust-constr"` (supports equality and inequality constraints).
:::
...
::: {.aside}
SciPy's `minimize()` also supports other methods including `"Powell"` (gradient-free direction set), `"SLSQP"` (constrained optimization), and several trust-region variants. See the [SciPy docs](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.minimize.html) for a full list.
:::
...
* R includes `"Brent"` for efficient 1-dimensional optimization (SciPy uses `optimize.minimize_scalar()` for this)

* The `optimx` packages provide access to additional methods beyond those in `optim()` - many additonal packages available on CRAN
...
## Profiling - BFGS (cProfile) {.scrollable}
...
<iframe data-src="Lec22_bfgs_quad.html" width="100%" height="500px" style="border:1px solid;border-radius: 5px;" data-external="1">
</iframe>
...
<iframe data-src="Lec22_nm_quad.html" width="100%" height="500px" style="border:1px solid;border-radius: 5px;" data-external="1">
</iframe>
...
::: {.aside}
::: {.fragment}
When $x = 0$ and $\mu = 0$: the gradient $\nabla f(x) = -f(x)\,\Sigma^{-1}(x - \mu) = 0$ 
:::
:::
...
```{python}
#| echo: false
df_no_cg = df.query("method != 'CG'")
g = sns.catplot(
    data=(df_no_cg[["name","method","time"]]
        .explode("time")
    ),
    y="method",
    x="time",

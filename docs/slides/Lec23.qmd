...
# From last time <br/> Maximum Likelihood example
...
## A regression example {.scrollable}
...
sgd_rep_time = time.perf_counter() - t0
sgd_lm_rep["x"][-1]
```
```{python}
t0 = time.perf_counter()
...
sgd_worep_time = time.perf_counter() - t0
sgd_lm_worep["x"][-1]
```
:::
...
Markdown(
  "| Method | Time / Epoch |\n"
  "|--------|-----------:|\n"
  f"| GD | {gd_time/20:.3f}s |\n"
  f"| SGD (replacement) | {sgd_rep_time/20:.3f}s |\n"
  f"| SGD (w/o replacement) | {sgd_worep_time/20:.3f}s |\n"
)
```
...
Markdown(
  ...
  "|--------|-----------:|\n"
  f"| GD | {gd_time/3:.3f}s |\n"
  f"| SGD (replacement) | {sgd_rep_time/3:.3f}s |\n"
  f"| SGD (w/o replacement) | {sgd_worep_time/3:.3f}s |\n"
)
```
:::
...
for size in sizes:
  ...
  mbgd_time[size] = time.perf_counter() - t0
```

```{python}
#| echo: false
for size in sizes:
  print(f"Batch size: {size} ({mbgd_time[size]/3:.3f}s / epoch)")
  print(mbgd[size]["x"][-1])
  print("")
...
for size in sizes:
  rows += f"| MBGD ({size}) | {mbgd_time[size]/3:.3f}s |\n"
Markdown(rows)
```
